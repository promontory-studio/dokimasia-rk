// A real HTTP server that speaks just enough of OpenAI's /v1/chat/completions to test an adapter
// over the wire: it records every request and answers with whatever the test queued, either a JSON
// completion or a list of SSE chunks.
//
// Published rather than kept in tests/ because every consumer of this harness needs it: a probe is
// only trustworthy if its scoring was proven offline before anything was billed, and that needs a
// server that can be told to fail on command.
import { createServer, type Server } from "node:http";
import type { AddressInfo } from "node:net";

export type FakeReply = { json: unknown; status?: number } | { sse: unknown[] };
